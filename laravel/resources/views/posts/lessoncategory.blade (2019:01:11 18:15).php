            ...
            <table class="table lessontable"><tbody>
                @forelse ($posts as $post)
                  <tr><td class="lessontitle">
                    ...
                    {{-- ログインしていない時 --}}
                    	@if ($post->usersonly == 0)
                        	<a href="{{ action('PostsController@lessonshow', $post->id) }}"><i class="fa fa-youtube-play"></i>　{{ $post->title }}</a>　<label class="label label-info">一般公開中</label>
                        ...
                        @else
                        	<i class="fa fa-lock gray" aria-hidden="true"></i>　<a href="{{ action('PostsController@lessonshow', $post->id) }}">{{ $post->title }}</a>　<label class="label label-primary">会員専用</label>
                        @endif
                    @else
                    {{-- ログインしている時 --}} 
                       	@if ($post->usersonly == 0 || $post->usersonly == 1 )
                        	<a href="{{ action('PostsController@lessonshow', $post->id) }}"><i class="fa fa-youtube-play"></i>　{{ $post->title }}</a>
                        @else
					  <i class="fa fa-exclamation-triangle gray" aria-hidden="true"></i>　<span class="gray">{{ $post->title }}</span>　<label class="label label-gray">準備中</label>
                        @endif
                    @endif
                    ...
                    <div class="media lessonbody">
                      <div class="media-left">
                        @if ($post->imageflag == 1)
                            <a href="{{ action('PostsController@lessonshow', $post->id) }}"><img class="postimage media-object" src="/image/bui{{ $post->id }}.png" alt="部位{{ $post->id }}"></a>
                        @endif
                      </div>
                      <div class="media-body">
                        <a href="{{ action('PostsController@lessonshow', $post->id) }}">{!! nl2br(e($post->body)) !!}</a>
                      </div>
                    </div>  
